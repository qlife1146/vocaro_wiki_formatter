...
-   v1.0.0 웹(2022.10.25)
    ...
    -   v1.10.1 웹(2023.08.05)
        -   `Clipboard API`가 적용되지 않아 구형 방식으로 롤백.
        -   `맞춤법 수정용` 칸을 수정하면 실시간으로 `완성 가사`에 적용됩니다.
    -   v1.11.0 웹(2023.09.06)
        -   유튜브 `공유` 버튼을 누르면 나오는 URL 주소가 왠지 모르겠지만 바뀌었습니다. 거기에 대응하도록 했습니다.
        -   유튜브 `공유` 버튼에서 '시작 시간'이 혹시나 눌릴 수도 있으니 이것도 대응하도록 했습니다.
        -   자기가 좋아하는 목록에서 작업을 할 수도 있잖아요. 나중에 볼 영상 같은 곳에서요. 'list'와 'index'가 붙어있어도 작동하도록 했습니다.
        -   원래 빈 칸이 있어도 형식이 완성되었으나 코드 변경 과정에서 누락됐었습니다. 이번 패치에 다시 작동하도록 변경했습니다.
        -   '비디오 타입'이 '니코동'일 때 `정보까지 변환`을 해도 경고창이 뜨지 않던 문제를 수정했습니다.
        -   작곡가, 작사가, 가수가 없을 때 경고창이 뜨지 않던 문제를 수정했습니다.
    -   v1.11.1 웹(2023.09.06)
        -   유튜브 주소 형식 코드 재작성
    -   v1.11.1d 웹(2023.09.12)
        -   맞춤법 수정용 텍스트 박스에 있던 문제 수정
        -   reformatting.js 파일에 주석 처리된 문단 삭제
        -   특정 상황에서 유튜브 링크가 제대로 작성되지 않는 문제 수정
        -   **`초기화` 버튼을 누를 때 추가된 텍스트 박스는 그대로 남는 문제 확인.**
    -   v1.12.0(2023.10.01)
        -   `맞춤법 검사용 가사` 버튼을 누를 때마다 맞춤법 수정용 텍스트 박스에 같은 가사가 중첩해 생기던 버그를 수정.
        -   기존에는 `초기화` 버튼을 누를 때 추가된 작곡가, 작사가, 가수 박스가 지워지지 않았으나 방법을 몰라 아예 새로운 방식을 적용.
        -   기존에 리스트 형식이었던 '비디오 타입'을 라디오 버튼 형식으로 바꾸었습니다. ~~클릭 한번이 줄었습니다.~~
        -   아티스트 리스트 업데이트
    -   v1.12.1(2023.10.03)
        -   유튜브 형식의 영상 아이디에 타임 스탬프가 포함되어 있을 때 삭제하도록 기능 추가.
