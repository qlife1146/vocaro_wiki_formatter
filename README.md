...
-   v1.0.0 웹(2022.10.25)
    ...
    -   v1.9.0 웹(2023.03.23)
        ...
        -    `변환` 버튼은 필요가 없을 것 같아서 삭제를 했습니다.
        -    또한, UI 최신 트렌드를 따라 `초기화` 버튼과 `정보까지 변환` 버튼의 위치를 바꿨습니다. 하지만 1.8.0에서 추가한 초기화 경고 단계가 있으니 불의의 사고는 없을 거라 믿습니다.~~아니 그 와중에 윈도우는 순서가 확인/취소네, 맥은 취소/확인이고~~
